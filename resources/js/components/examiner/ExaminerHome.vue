    ...
    <div class="container">
        <div class="jumbotron jumbotron-fluid">
            <div class="col-md-10 offset-md-1">
                <h1>Welcome, {{ currentUser.username }}! [{{currentUser.education.name}}]</h1>
        ...
        <div class="d-flex flex-row flex-wrap d-inline justify-content-around mt-2">
            <!-- Step One -->
            <div class="card" v-if="stepIndex == 0">
                <div class="card-body">
                    ...
                    <div class="input-group sticky-top mt-2">
                        ...
                        <select class="custom-select col-md-2" style="border: none;" v-model="searchExamEducation">
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover table-large">
                            ...
                            <tbody>
                                <tr class="pointer" v-for="(exam, examIndex) in getExamList" v-on:click="selectExam(examIndex)">
                                    <th>{{examIndex+1}}</th>
                                    <td>{{exam.year}}</td>
                                    <td>{{exam.name}}</td>
                                    <td>{{exam.education.name}}</td>
                                    <td>
                                        <i class="pointer fas fa-eye mr-2" v-on:click="redirect('ExamPreview', exam.id)"></i>
                                    </td>
                                </tr>
            ...
            <!-- Step Two -->
            <div class="card" v-if="stepIndex == 1">
                <div class="card-body">
                    <h5 class="card-title">
                        <button class="btn btn-outline-primary" v-on:click="stepBack()"><i class="fas fa-long-arrow-alt-left"></i> Back</button>
                        Students
                    </h5>
                    <div class="input-group sticky-top mt-2">
                        <div class="input-group-prepend">
                        ...
                        <input type="text" class="form-control" style="border: none;" v-model="searchStudent" placeholder="Search Exam">
                    </div>
                    <div class="table-responsive">
                        <table class="table table-hover table-large">
                            <thead>
                            ...
                            <tbody>
                                <tr class="pointer" v-for="(student, studentIndex) in getStudentList" v-on:click="selectStudent(studentIndex)">
                                    <th>{{studentIndex+1}}</th>
                                    <td>{{student.name}}</td>
                                    <td>{{student.ov}}</td>
                                    <td>{{student.year}}</td>
                                </tr>
                                <tr v-if="getStudentList.length <= 0">
                                    <th>-</th>
            ...
            <!-- Step Three -->
            <div class="card" v-if="stepIndex == 2">
                <div class="card-body">
                    <h5 class="card-title">
                        <button class="btn btn-outline-primary" v-on:click="stepBack()"><i class="fas fa-long-arrow-alt-left"></i> Back</button>
                        Overview:
                    ...
                    <p class="card-text">
                        <b>Exam Name:</b> {{selectedExam.name}}<br />
                        <b>Exam Year:</b> {{selectedExam.year}}<br />
                        <b>Student Name:</b> {{selectedStudent.name}} ({{selectedStudent.ov}})<br />
                        <b>Student Year:</b> {{selectedStudent.year}}
                        <br />
                    </p>
                    <button class="btn btn-outline-primary" v-on:click="startExam()" v-bind:disabled="disableStartExam">Start Exam</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    export default {
        ...
        data() {
            return {
                searchExam: '',
                searchExamEducation: -1,
                searchStudent: '',
                stepIndex: 0,
                disableStartExam: true,
        ...
        computed: {
            getExamList() {
                ...
                return this.exams.filter(function(exam){
                    if(self.searchExamEducation != -1){
                        return exam.education_id == self.searchExamEducation;
                    }else{
        ...
        methods: {
            ...
            stepBack(){
                ...
                if(this.selectedExam != null && this.selectedStudent != null){
                    this.disableStartExam = false;
                }else{
                    this.disableStartExam = true;
                }
            },
            getExams() {
                var self = this;
                new Promise((res, rej) => {
                    axios.post('/api/home/getExams', {id:this.currentUser.education_id})
                    .then((response) => {
                        this.exams = response.data;
                    })
                    .catch((err) => {
                        self.createLog(self.currentUser, `Tried to load exams but an error occured`);
                        self.$noty.error("Exams cannot be pulled");
                    })
                });
            },
            getEducations(){
                var self = this;
                new Promise((res, rej) => {
                    axios.post('/api/home/getEducations', {'id': this.currentUser.domain_id})
                    .then((response) => {
                        this.educations = response.data;
                    })
                    .catch((err) =>{
                        self.createLog(self.currentUser, `Tried to load educations but an error occured`);
                        self.$noty.error("Educations cannot be pulled");
                    })
                });
            },
            ...
            getStudents() {
                var self = this;
                new Promise((res, rej) => {
                    axios.post('/api/home/getStudents', {'domain_id': this.currentUser.domain_id, 'education_id': this.currentUser.education_id, 'exam_id':this.selectedExam.id})
                    .then((response) => {
                        this.students = response.data;
                    })
                    .catch((err) => {
                        self.createLog(self.currentUser, `Tried to load students but an error occured`);
                        self.$noty.error("Students cannot be pulled");
                    })
                });
            },
            selectExam(index) {
            ...
            startExam(){
                ...
                .confirm({
                    title: "Warning",
                    body: "Are you sure you want to start this exam?",
                })
                .then(dialog => {
                    var data = {
                ...
                new Promise((res, rej) => {
                    ...
                    .then((response) => {
                        if(response.data != null) {
                            ...
                            .confirm({
                                title: "Warning",
                                body: "There is already an exam in progress. Please finish the exam before you can start a new one.",
                            })
                            .then(dialog => {
                                this.$router.push({ name: 'ExamSheet', params: { 'id': response.data.id }});
                            })
                            .catch(() => {
                              this.delInstance(response.data.id);
                            });
                        }
                    })
                    .catch((err) =>{})
            ...
            delInstance(id){
                ...
                new Promise((res, rej) => {
                    ...
                    .then((response) => {
                        self.$noty.success("Successfully deleted instance");
                    })
                })
            },

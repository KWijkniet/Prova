        ...
        <div class="login-body">
            ...
            <div class="wrapper">
                ...
                <form @submit="checkForm" novalidate="true">
                    <h3>Register</h3>
                    <div class="mail">
                        <input type="text" v-model="form.username" class="form-control" placeholder="User Name" required>
                    ...
                    <div class="passwd">
                        <input type="password" v-model="form.confirm" class="form-control" placeholder="Confirm Password" required>
                        <label for="password">Confirm Password:</label>
                    </div>
                    <div class="form-group row" v-show="error != null">
                        <p class="error">
                            {{ error }}
                        </p>
                    </div>
                    <div class="submit">
                        <button v-show="!showLoadAnim" v-on:click="showLoadAnim=true" type="submit" id="loginButton" class="dark">Register</button>
                        <div v-show="showLoadAnim" id="loader" class="loader loader--style3" title="2">
...
<script>
    ...
    export default {
        ...
        methods: {
            createUser() {
                var self = this;
                this.$data.form.email = this.$data.form.email.toLowerCase();
                new Promise((res, rej) => {
                    axios.post('/api/auth/register', {data: this.$data.form, token: this.$route.query.invitation_token})
                    .then((response) => {
            ...
            checkForm(e){
                ...
                if (this.$data.form.username.length == 0) {
                   hasErrors = true;
                    ...
                    } else if (this.$data.form.email.length == 0) {
                   hasErrors = true;
                        this.error = "Email required.";
                      } else if (this.$data.form.password.length == 0) {
                   hasErrors = true;
                        this.error = "Password required.";
                      } else if (this.$data.form.confirm.length == 0) {
                   hasErrors = true;
                        this.error = "Password confirmation required.";
                      }
